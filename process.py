"""
Inspired by Monoloco
Unnormalized_bi function has been modified
"""
import json
...
def cauchy_sampling(outputs, n_samples):
    ...
    cauchy = torch.distributions.cauchy.Cauchy(mu, bi)
    xx = cauchy.sample((n_samples,)).to(device)
...
def unnormalize_bi(outputs):
    ...
    """
    author: N. Vamsy Krishna
    mail:nandurikrishna6@gmail.com
    """
    
    outputs[:, 1] = torch.sqrt(torch.abs(outputs[:, 1])) * outputs[:, 0]
    return outputs
